...
{
    var calculationHistories = db.CalculationHistories.ToList();

    foreach (var calculationHistory in calculationHistories)
    {
        this.CalculationHistoryListView.Items.Add($"{calculationHistory.DateTime}: {calculationHistory.Calculation}");
    }
}
...
void OnClearHistory(object sender, EventArgs e)
{
    ...
    {
        db.Database.Migrate(); // Ensure the database has been created

        // Delete all records from the CalculationHistory table
        var history = db.CalculationHistory.ToList();
        db.CalculationHistory.RemoveRange(history);
        db.SaveChanges();
    }
}
